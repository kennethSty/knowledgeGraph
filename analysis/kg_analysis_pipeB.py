...
CALL gds.betweenness.stream('pipeB_graph')
YIELD nodeId, score
...
axes.set_xlabel('Nodes', fontsize=14, color='white')
axes.set_ylabel('Betweenness Centrality', fontsize=14, color='white')
axes.grid(alpha=0.6, linestyle='--')
axes.set_xticklabels(node_short_nodes, color='white')
axes.set_yticklabels([round(node) for node in node_short_scores[::-1]], color='white')
axes.tick_params(axis='x', rotation=45)

plt.tight_layout()
plt.savefig('nodes_btw_centrality_pipeB.png', dpi=300, transparent=True)

## AVG SHORTEST PATH LENGTH
avg_shortest_path_len = kg.query("""
CALL gds.allShortestPaths.stream('pipeB_graph')
...
axes[0][1].set_xlabel('Shortest Path Lengths', fontsize=14, color='white')
axes[0][1].grid(alpha=0.6, linestyle='--')
axes[0][1].tick_params(axis='x', colors='white')
axes[0][1].tick_params(axis='y', colors='white')
axes[0][0].set_title(title='Shortest Paths btw all Pages')

plt.tight_layout()
plt.savefig('shortest_paths_pipeB.png', dpi=300, transparent=True)

print(f"Avg. shortest path length: {avg_shortest_path_len}")
print(f"Max shortest path length{max_shortest_path_len}")
