...
The above limitations also impacted the evaluation of the pipeline. Starting with the obvious, the amount of pages processed to evaluate the different pipeline configurations is very small and cannot reflect the pipeline's actual application setting. However, exactly these scenarios might be of particular interest especially when using the generated knowledge graph as a graph-vector store for retrieval-augmented generation.

Also, besides its small scale, our evaluation approach relied on the MeSH thesaurus as a ground truth database for medically relevant node entities. As emphasized earlier in the section on our results, the thesaurus includes terms that go beyond the typical medical scope. Even though this is not in line with the initial intuition about a ground truth database of medically relevant entities, we refrained from further cleaning it. We made this decision for the following reasons. First, the thesaurus is carefully curated and created by a group of medical experts, while we lack any medical training. Further, we assume that each node of the thesaurus is included for a reason. Thus, nodes that go beyond the medical scope might indicate the relevance of such entities to create a complete knowledge graph in the first place. In such a case, the inclusion of these entities in our MeSH evaluation database and the metrics we compute on top of it might more accurately assess the constructed graph than a database of exclusively medical terms. Nevertheless, this is only an assumption. Future research could clarify this question further by explicitly characterizing the nodes, entities and relationships relevant to an MKG from the perspective of medical professionals. 

Another limitation of our evaluation resides in the limited scalability of our qualitative shortest-path analysis to larger application scenarios. While our proposed evaluation method helps to create a deeper understanding during development a more robust and metric-based approach might be needed for large-scale application scenarios. In developing such a scalable connectivity evaluation strategy, we anticipate another opportunity for future work. For this purpose, one could create an evaluation dataset by following hyperlinks within the HTML code of the Wikipedia pages. The decision of whether a link between two pages is medically relevant or not could be made by a large language model. After constructing the dataset of evaluation pages using the previously described process, the graphs can be generated using a graph construction pipeline. During the evaluation phase, the presence of shortest paths between linked pages can be analyzed. In an advanced methodology, the binary evaluation of the "existence/non-existence" of these paths can be enhanced by parsing the nodes and edges along the shortest path into textual form. This textual representation can then be assessed again by another language model to determine if the expected link is accurately reflected by the shortest path or not. 
