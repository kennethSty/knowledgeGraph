...
Next, we assess H10 and H11 visually given the shortest paths that connect the different page nodes. First, it can be observed, that while there exist several paths between all pages (forward and backward) in graph B. Unexpectedly not one path exists in graph C. In graph A, only one path exists, which is not one of the paths we expected. Consequently, H10 and H11 need to be rejected. 

The existence of the anticipated links "Lissencephaly - Ryanodine Receptors" and "Anthrax - Dermatitis" in graph B gives pipeline B the appearance of superiority in linkage detection among the other configurations. However, this changes with a closer look at the detected paths in the following figure. 
