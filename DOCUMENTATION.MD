...
Besides these different components, that make up a successful knowledge graph construction pipeline, another factor for success is the relevance of the constructed graph with respect to its application scenario. Due to the lack of a ‘one-size fits all’ schema or graph for every setting, a graph needs to be constructed for its application domain specifically [^26]. 



## Objectives
In light of this background, the objectives of this project were twofold. Foremost, the objective was one of exploration. We aimed to explore and report on:
  1. the feasibility of implementing a pipeline via langchain for constructing a specialized medical knowledge graph from unstructured German documents
  2. a possible evaluation strategy for specialized knowledge graphs in the medical domain 
  3. highlighting different approaches for extending and improving a Graph Construction pipeline
