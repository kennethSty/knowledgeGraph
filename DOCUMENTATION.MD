...
### Evaluation Dataset
The evaluation dataset consisted of five randomly selected Wikipedia pages that make up the evaluation set covered the following topics: 
...
We base H4 on the assumption that the pipeline with the node filter together with (or without) the medical prompt is engineered the most towards returning medical entities. Consequently, we expect a larger percentage in MeSH matches in graphs A and C. 
...
While being connected to the issue of detecting relevant nodes, the major challenge in automatic knowledge graph construction resides in detecting domain-specific links across pages without introducing noise in the form of irrelevant links. Pipeline B was moderately successful in balancing this trade-off. While identifying the most relevant nodes, the percentage of these nodes compared to the overall amount of detected nodes is small. Likewise, it detects the most medically relevant links. But at the same time, it also contains spurious links without medical relevance. Pipeline A achieves a better balance. While it detects fewer nodes and links across pages, it does so by adding less noise to the graph. This hints towards the necessity of customizing a knowledge graph construction to its specific domain. However, our results also highlight that additional work is needed to finetune these components in order to achieve the best balance or even break the previously mentioned trade-off. 

#### Results
The following figure illustrates the results of the graph construction and evaluation runs. 
...
While it appears the second filter chain is of critical importance to ensure domain specificity, the results show it requires further tuning as in absolute numbers pipeline B (without the filter) identifies 101 more entities matched in the MeSH database. A similar drop can be observed from graph B to C. In other words, when using the filter, we increase precision, but we reduce recall as indicated by the 100+ nodes that pipelines A and C filtered out. Consequently, H6 and H7 have to be rejected. Possible explanations for this behaviour could be the simple input prompt used for the filter chain, which was necessary to ensure stable outputs of the quantized Llama3 Sauerkraut model. Thus, a possible improvement resides in switching choosing a different technological set-up to deploy Lllama3 Sauerkraut without quantization. 
...
Next, we assess H10 and H11 visually given the shortest paths that connect the different page nodes. First, it can be observed, that while there exist several paths between all pages (forward and backward) in graph B. Unexpectedly not one path exists in graph C. In graph A, only one path exists, which is not one of the paths we expected. Consequently, H10 and H11 need to be rejected. Especially intriguing is the existence of a link in graph A but the absence of a link in graph C. Together with the previously discussed MeSH precision, this highlights the importance of aligning the input prompt and node filter to the same domain-specific extraction purpose in order to increase the likelihood of detecting relevant nodes and edges. Similarly astounding is the large difference in links between graph B and graph A. To assess whether the detected links between"Lissencephaly - Ryanodine Receptors" and "Anthrax - Dermatitis" in graph B indeed match our expected relationships between the pages, we assess the shortest paths visually in the following figure. 
